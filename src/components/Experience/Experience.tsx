...
export default function Experience() {
  ...
  return (
    <section className="experience" id="resume" aria-labelledby="experience-title">
      <div className="container">
        <div className="experience__head reveal" ref={headingRef}>
          <p className="eyebrow">{experience.eyebrow}</p>
          <h2 id="experience-title">
            <AccentText parts={experience.title} />
          </h2>
        </div>

        <div className="experience__timeline reveal-stagger" ref={listRef}>
          {experience.items.map((item, i) => (
            <div className={`experience__row ${i % 2 === 1 ? 'is-reversed' : ''}`} key={item.company}>
